...
*********************************************************/
    ...
    function fonctionLectureConfigCustom(){
        ...
        // Lecture du nb de colonnes à afficher et stockage dans le html (#parametresConfig)
            $nbColonnes = $sessionConfigCustom.options.nbColonnes;
        ...
        // Lecture et affichage dans le html "#ZoneSelectionPictos" des contenus des colonnes + stockage dans le html (#parametresConfig) + injection des popups correspondants
            ...
            for ($i=0; $i<10; $i++){
                $colonne = $sessionConfigCustom.colonnes[$i];
        ...
        // Lecture et stockage des largeurs de colonnes
            ...
            for ($i=0; $i<10; $i++){
                $largeurColonne = $sessionConfigCustom.largeurColonnes[$i];
        ...
        // Lecture et affichage dans le html "#ZoneSelectionPictos" des contenus des boutons + stockage dans le html (#parametresConfig) + injection des popups correspondants   
            ...
            for ($i=0; $i<10; $i++){
                $bouton = $sessionConfigCustom.boutons[$i];
...
*********************************************************/
    function fonctionAffichageColonne(categorie,largeur){
        ...
        $colonne = $('.ColonnePictos'+categorie);
        $popup = $('.Popup'+categorie);
    ...
    function fonctionRemplissagePopup($popup,$BanquePictosCategorie,$couleur){
        ...
        $NbPictos1 = $BanquePictosCategorie.pictos1.length;
        $NbPictos2 = $BanquePictosCategorie.pictos2.length;
        $NbPictos3 = $BanquePictosCategorie.pictos3.length;
        
        if ($('#complexite').text()==1){$NbPictos = $NbPictos1;}
        if ($('#complexite').text()==2){$NbPictos = $NbPictos1+$NbPictos2;}
        if ($('#complexite').text()==3){$NbPictos = $NbPictos1+$NbPictos2+$NbPictos3;}
        // injection des pictos 1 dans le popup
        for (i=0; i<$NbPictos1; i++){
            $mot=$BanquePictosCategorie['pictos1'][i];
...
*****************************************************/
        ...
        $dimension = 98/$('#nbColonnes').text();
        //console.log('dimension : '+$dimension);

        // récupération du nb de pictos par colonne :
        $hauteur = $('#hauteurColonnes').text();
